...
function HistoryOrder() {
  ...
  async function getTransaction() {
    const response = await fetch("http://143.198.222.47:10003/transaction", {
      headers: {
            ...
            {isLoading || err
              ...
              : order.map((item) => {
                  return (
                    <>
                      <div
                        className="flex gap-5 bg-[#E8E8E8]/30 p-4 rounded-lg"
                        key={item.id}
                      >
                        ...
                        <div className="">
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
                            <div className="flex flex-col gap-3 py-3">
                              ...
                              <div className="font-bold py-2 text-[#0B132A]">
                                #{item.NoOrder}
                              </div>
                            </div>
                            <div className="flex flex-col gap-3 py-3">
                          ...
                          <Link
                            to={`/detail-order/${item.NoOrder}`}
                            className="text-sm underline text-[#FF8906] font-medium"
                          >
                            Views Order Detail
                          </Link>
                        </div>
