...
function ProductList() {
  ...
  async function products() {
    const dataProducts = await fetch("http://143.198.222.47:10003/products/", {});
  ...
  async function filterProducts(e) {
    ...
    const dataProducts = await fetch(
      `http://143.198.222.47:10003/products/filter/?title=${search}&page=${page}`,
      {}
    );
    const listProduct = await dataProducts.json();
    setProduct(listProduct.result);
  ...
  async function allProduct() {
    const dataProducts = await fetch(
      `http://143.198.222.47:10003/products/filter/?title=${search}&limit=1000&page=${page}`,
      {}
    );
    const listProduct = await dataProducts.json();
  ...
  function removeData(id) {
    fetch("http://143.198.222.47:10003/products/" + id, {
      method: "DELETE",
    })
      .then(() => {
        console.log("removed");
          ...
          <div className="border-l w-full min-h-screen pt-6 pl-7 pr-20 flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-5 border rounded-md p-5">
                  ...
                  <tbody>
                    {product.map((item) => (
                      <tr key={item.id} className="border-t text-[#4F5665]">
                        ...
                        <td className="text-center px-3 py-3">{item.title}</td>
              ...
              <div className="flex justify-between text-[#4F5665]">
                <div>
                  <p>
                    Show {totalProduct > 0 ? startIndex : 0} - {endIndex} of{" "}
                      ...
                      <button
                        onClick={() =>
                          setPage(
                            Math.min(
                              page + 1,
                              Math.ceil(totalProduct / itemPerPage)
                            )
                          )
                        }
                        className="hover:text-[#FF8906]"
